...
export class CreateHmacTests {
  createHmac_sha256_produces_correct_digest(): void {
  ...
  createHmac_with_byte_array_key(): void {
  ...
  createHmac_update_with_bytes_works(): void {
  ...
  createHmac_digest_as_bytes_works(): void {
    const hmac = createHmac("sha256", "key");
    hmac.update("test");
    const digest = hmac.digest();
    Assert.Equal(32, digest.length);
