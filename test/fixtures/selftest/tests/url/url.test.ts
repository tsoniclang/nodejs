...
export class URLTests {
  ...
  URL_toString_ShouldReturnSerializedURL(): void {
    const parsed = new URL("https://example.com/path");
    Assert.Equal(parsed.href, parsed.toString());
  }

  URL_toJSON_ShouldReturnSerializedURL(): void {
    const parsed = new URL("https://example.com/path");
    Assert.Equal(parsed.href, parsed.toJSON());
  }

  URL_canParse_WithValidURL_ShouldReturnTrue(): void {
    Assert.True(URL.canParse("https://example.com"));
  }

  URL_canParse_WithInvalidURL_ShouldReturnFalse(): void {
    Assert.False(URL.canParse("not a url"));
  }

  URL_canParse_WithRelativeURLAndBase_ShouldReturnTrue(): void {
    Assert.True(URL.canParse("/path", "https://example.com"));
  }

  URL_parse_WithValidURL_ShouldReturnURL(): void {
    const parsed = URL.parse("https://example.com");
    Assert.NotNull(parsed);
    Assert.Equal("https://example.com/", parsed!.href);
  }

  URL_parse_WithInvalidURL_ShouldReturnNull(): void {
...
export class URLSearchParamsTests {
  URLSearchParams_Constructor_Empty_ShouldCreateEmpty(): void {
    const params = new URLSearchParams();
    Assert.Equal(0, params.size);
  }

  URLSearchParams_Constructor_WithString_ShouldParse(): void {
  ...
  URLSearchParams_Constructor_WithQueryString_ShouldParseWithoutQuestionMark(): void {
    const params = new URLSearchParams("?foo=bar");
    Assert.Equal("bar", params.get("foo") as string);
  }

  URLSearchParams_append_ShouldAddParameter(): void {
    const params = new URLSearchParams();
    params.append("key", "value");
    Assert.Equal("value", params.get("key") as string);
  }

  URLSearchParams_append_AllowsDuplicates(): void {
    const params = new URLSearchParams();
  ...
  URLSearchParams_set_ShouldReplaceExistingValues(): void {
  ...
  URLSearchParams_get_WithNonExistentKey_ShouldReturnNull(): void {
    const params = new URLSearchParams();
    Assert.Null(params.get("nonexistent"));
  }

  URLSearchParams_getAll_ShouldReturnAllValues(): void {
    const params = new URLSearchParams("key=1&key=2&key=3");
    const values = params.getAll("key");
    Assert.Equal(3, values.length);
  }

  URLSearchParams_getAll_WithNonExistentKey_ShouldReturnEmptyArray(): void {
    const params = new URLSearchParams();
    Assert.Equal(0, params.getAll("nonexistent").length);
  }

  URLSearchParams_has_WithExistingKey_ShouldReturnTrue(): void {
    const params = new URLSearchParams("key=value");
    Assert.True(params.has("key"));
  }

  URLSearchParams_has_WithNonExistentKey_ShouldReturnFalse(): void {
    const params = new URLSearchParams();
    Assert.False(params.has("key"));
  }

  URLSearchParams_has_WithValue_ShouldCheckBothKeyAndValue(): void {
  ...
  URLSearchParams_delete_ShouldRemoveParameter(): void {
    const params = new URLSearchParams("key=value");
    params.delete("key");
    Assert.False(params.has("key"));
  }

  URLSearchParams_delete_WithValue_ShouldRemoveSpecificPair(): void {
    const params = new URLSearchParams("key=value1&key=value2");
  ...
  URLSearchParams_sort_ShouldSortByKeys(): void {
    const params = new URLSearchParams("c=3&a=1&b=2");
    params.sort();
    Assert.Equal("a=1&b=2&c=3", params.toString());
  }

  URLSearchParams_keys_ShouldReturnAllKeys(): void {
    const params = new URLSearchParams("a=1&b=2&c=3");
    const keys = params.keys();
    Assert.Equal(3, keys.length);
  }

  URLSearchParams_values_ShouldReturnAllValues(): void {
    const params = new URLSearchParams("a=1&b=2&c=3");
    const values = params.values();
    Assert.Equal(3, values.length);
  }

  URLSearchParams_forEach_ShouldIterateOverAllPairs(): void {
  ...
  URLSearchParams_toString_ShouldReturnQueryString(): void {
    const params = new URLSearchParams("a=1&b=2");
    Assert.Equal("a=1&b=2", params.toString());
  }

  URLSearchParams_toString_WithEmptyParams_ShouldReturnEmptyString(): void {
    const params = new URLSearchParams();
    Assert.Equal("", params.toString());
  }

  URLSearchParams_size_ShouldReturnCorrectCount(): void {
    const params = new URLSearchParams("a=1&b=2&c=3");
    Assert.Equal(3, params.size);
  }

  URLSearchParams_size_AfterModifications_ShouldUpdate(): void {
    const params = new URLSearchParams();
    Assert.Equal(0, params.size);
